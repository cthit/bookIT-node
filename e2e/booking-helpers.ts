...
export async function fillBooking(page: Page, title: string) {
  ...
  await page.getByLabel("Phone number", { exact: true }).fill("0701234567");
  await expect(page.getByLabel("Booking as", { exact: true })).toHaveValue("");
  await expect(page.getByRole("checkbox", { name: "Storhubben", exact: true })).toBeChecked();
  await page.getByLabel("Booking as", { exact: true }).selectOption("digit");
