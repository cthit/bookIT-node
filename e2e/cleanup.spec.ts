...
for (const trigger of ["startup", "command"] as const) {
  test(`${trigger} cleanup removes old contact data while retaining current bookings`, async ({
    page,
    environment,
  }) => {
    ...
    if (trigger === "startup") {
      await environment.restartBookit();
    } else {
